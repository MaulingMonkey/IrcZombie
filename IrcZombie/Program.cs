...
namespace IrcZombie {
	class Program {
		...
		static void Relaunch() {
			...
			var pcopy = Process.Start( "xcopy", String.Format( "/E \"{0}\" \"{1}\\\"", Path.GetDirectoryName(OriginalPath), tempdir ) );
			...
			if ( Connection != null && Connection.Socket.Connected ) {
				...
				var pclone = Process.Start( newexe, string.Format( "--original={0} --pipe={1}", OriginalPath, pipe.GetClientHandleAsString() ) );
		...
		static void Main( string[] args ) {
			...
			if (args.Any(arg=>arg.StartsWith("--pipe="))) {
				var pipe   = new AnonymousPipeClientStream(args.First(arg=>arg.StartsWith("--pipe=")).Remove(0,"--pipe=".Length));
